...
Keys are generated automatically by `InstanceManager.register()`. If you need to generate them manually (e.g., for pre-provisioning or scripting), use Node.js `crypto` or Python `cryptography` directly:
...
To check that stored credentials are valid without a full connection, use `ping` which performs a lightweight handshake:
...
const manager = new InstanceManager();
try {
  const { latencyMs, version } = await manager.ping(instance.id);
  console.log(`Credentials OK — gateway v${version}, ${latencyMs}ms`);
} catch (err) {
